...
const Registro = ({ onRegistroSuccess, irALogin }) => {
    const [form, setForm] = useState({
        ...
        correo: '',
        password: '',
        confirmar_password: '' // <-- Nuevo campo en el estado
    });
    ...
    const validarFormulario = () => {
        ...
        // <-- Nueva validación para coincidencia de contraseñas
    ...
    const manejarRegistro = async (e) => {
        ...
        // Copiamos los datos del formulario
        const datosAEnviar = { ...form };
        
        // Eliminamos confirmar_password para no enviarlo al backend
        delete datosAEnviar.confirmar_password; 

        // 2. Formatear la fecha para Oracle (Asegurando DD/MM/YYYY con año de 4 dígitos)
        ...
        // 3. Unir el celular con el código regional antes de enviar
        ...
        try {
            ...
            // Redirigir o actualizar estado de éxito
    ...
    return (
        <div className="min-h-screen flex items-center justify-center bg-black p-4 font-sans relative overflow-hidden">
            ...
            <div className="bg-gray-900/80 backdrop-blur-xl p-8 sm:p-10 rounded-[2.5rem] shadow-2xl w-full max-w-md border border-white/5 max-h-[90vh] overflow-y-auto no-scrollbar relative z-10 animate-fade-in-up">
                ...
                {error && (
                    <div className="bg-red-500/10 border-l-4 border-red-500 text-red-400 px-4 py-3 rounded-xl mb-6 text-[10px] font-black uppercase flex items-center gap-2">
                        {error}
                    </div>
                )}
                ...
                <form onSubmit={manejarRegistro} className="space-y-4">
                    ...
                    <div className="grid grid-cols-3 gap-2">
                        <select
                            className="col-span-1 bg-black/50 border border-gray-700 text-white rounded-2xl px-2 py-4 focus:border-blue-500 outline-none text-sm appearance-none cursor-pointer"
                            value={codigoPais}
                            onChange={(e) => setCodigoPais(e.target.value)}
                        >
                            {CODIGOS_PAISES.map(p => (
                                <option key={p.code} value={p.code} className="bg-gray-900">{p.code} ({p.country})</option>
                            ))}
                        </select>
                        <input
                            type="tel"
                            className="col-span-2 w-full bg-black/50 border border-gray-700 text-white rounded-2xl px-5 py-4 focus:border-blue-500 transition-all text-sm outline-none"
                            placeholder="Celular"
                    ...
                    {/* <-- NUEVO BLOQUE: Dos campos de contraseña --> */}
                    <div className="grid grid-cols-2 gap-3">
                        <input 
                            type="password" 
                            className="w-full bg-black/50 border border-gray-700 text-white rounded-2xl px-5 py-4 focus:border-blue-500 transition-all text-sm outline-none" 
                ...
                <div className="fixed inset-0 z-50 flex items-center justify-center p-6 animate-fade-in">
                    ...
                    <div className="bg-gray-900 border border-white/10 w-full max-w-lg rounded-[2.5rem] p-8 relative z-10 shadow-2xl max-h-[80vh] overflow-hidden flex flex-col">
                        <header className="mb-6 flex justify-between items-center shrink-0 border-b border-white/5 pb-4">
                            ...
                            <button onClick={() => setVerModal(false)} className="text-gray-500 hover:text-white p-2 bg-white/5 rounded-full transition-colors">
                        ...
                        <div className="flex-1 overflow-y-auto no-scrollbar space-y-6 pr-2">
                            <section>
                                ...
                                <p className="text-gray-300 text-sm leading-relaxed">
                                    En cumplimiento con la **Ley N° 19.628 sobre Protección de la Vida Privada**, Alerta Digital informa que el tratamiento de sus datos personales se realizará con el único propósito de proveer seguridad heurística. No comercializamos su información.
                                </p>
                            </section>
                            ...
                            <section>
                                ...
                                <p className="text-gray-300 text-sm leading-relaxed">
                                    El acceso a lectura de mensajes tiene como fin exclusivo agilizar el motor de análisis para detección de fraudes. No se utiliza para vigilancia.
                                </p>
                            </section>
                            ...
                            <section>
                                ...
                                <p className="text-gray-300 text-sm leading-relaxed">
                                    Garantizamos la integridad de su información mediante encriptación de grado bancario en servidores Oracle Cloud, cumpliendo la Ley de Delitos Informáticos.
                                </p>
                            </section>
                            ...
                            <section>
                                ...
                                <p className="text-gray-300 text-sm leading-relaxed">
                                    Puede solicitar la eliminación definitiva de su cuenta en cualquier momento desde su perfil, ejerciendo sus derechos ARCO.
                                </p>
                            </section>
                        </div>
