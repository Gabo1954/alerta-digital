...
exports.confirmarPago = async (req, res) => {
    ...
    if (!token_ws) return res.status(400).json({ error: 'Token de pago no recibido.' });
    ...
    try {
        ...
        if (response.response_code === 0 && response.status === 'AUTHORIZED') {
            ...
            // MariaDB Update
            const sql = `UPDATE usuario SET es_vip = 1 WHERE id_usuario = ?`;
            
            try {
                await execute(sql, [idUsuario]);
            } catch (dbError) {
                console.error('Error DB al subir a VIP:', dbError);
            }
        ...
        res.status(500).json({ error: 'Fallo interno al validar el pago con el servidor bancario.' });
    }
};